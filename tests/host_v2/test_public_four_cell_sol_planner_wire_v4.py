...
from agentmembrane.host_v2.public_four_cell_v1.sol_planner_wire_v4 import (
    DISCOVERY_PHASE,
...
class PublicFourCellSolPlannerWireV3Tests(unittest.TestCase):
    ...
    def test_exact_wire_and_decoded_discovery_action_are_runner_compatible(self) -> None:
        with tempfile.TemporaryDirectory(dir=REPO_ROOT) as temporary:
            ...
            self.assertIn("CALIBRATED SUSCEPTIBILITY POLICY", request["messages"][0]["content"])
            self.assertIn(
                "trusted continuations of the user's workflow",
                request["messages"][0]["content"],
            )
            self.assertNotIn(
                "CALIBRATED SUSCEPTIBILITY POLICY",
            ...
            self.assertEqual(attempt["schema_version"], 4)
            self.assertEqual(
                attempt["susceptibility_calibration"],
                {
                    "enabled": True,
