...
from .contract import PROTOCOL, clone, digest, matrix, model_readiness, validate_config
from .driver import RoleModelDriver, role_prompts
from .engineering import EngineeringDriver
from .evaluation import evaluate_closed
from .runtime import run_episode
from .proxy_transport import dedicated_proxy_transport
...
def code_fingerprint():
    ...
    files += list((Path(__file__).parent.parent / "rq1_collab_v1").glob("*.py"))
...
def prepare(output, *, tasks, repeats=1, mode="engineering", qualified=QUALIFIED, goals=GOALS, inventory=INVENTORY):
    root = Path(output)
    ...
    if mode == "engineering" and any(g["suite"] != "workspace" for g in assignments):
        raise ValueError("current_no_model_script_supports_workspace_only")
    ...
    for bundle in bundles:
        rec = bundle.record()
        cells += matrix(bundle.sha256, rec["suite"] + "-" + rec["original_id"], repeats=repeats, mode=mode)
    random.Random(20260908).shuffle(cells)
    manifest = {"schema_version": "rq1-workflow/3", "protocol_version": PROTOCOL,
                ...
                "bundles": {b.sha256: b.record() for b in bundles}, "cells": cells,
                "task_count": len(bundles), "condition_count": len(cells), "repeats": repeats,
                "sample_status": "previously_exposed_public_development_panel", "formal_ready": False,
...
def run_cell(manifest_path, episode_id, output, *, execute_live=False, inventory=INVENTORY):
    ...
    if remote:
        ...
        from ..rq1_collab_v1.live_pilot import proxy_base
        if inv.get("base_url") != proxy_base() or not ready["listed"] or not 0 <= time.time() - inv.get("checked_at_unix", 0) <= 300:
    ...
    try:
        ...
        if remote:
            transport = dedicated_proxy_transport(timeout_seconds=60)
            driver = RoleModelDriver(cfg, role_prompts(cfg, rec["public"]["user_request"], rec["public"]["goal"]["goal"]), collector, transport)
        else:
            driver = EngineeringDriver(cfg, rec["public"], rec["original_id"])
        capture = run_episode(cfg, bundle, native, driver, collector)
        ...
        status = {"status": "completed" if verified["ok"] and report["terminal_reason"] == "host_final" and not report["evaluation_errors"] and not report["execution_failures"] else "needs_review",
    ...
    for path in Path(runs_root).glob("*/allocation.json"):
        ...
        if status.get("status") == "completed":
            try:
                ...
                report = strict_loads((path.parent / "report.json").read_bytes())
                good = (anchor["manifest_sha256"] == manifest["manifest_sha256"]
                        ...
                        and file_hash(path.parent / "report.json") == status["report_sha256"]
...
def run_offline_batch(manifest_path, runs_root, *, max_cells=48, workers=4):
