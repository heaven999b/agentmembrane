"""Read-only honest/malicious paired pilot analysis.
...
from ..rq1_collab_v1.audit import (
    _write_new, canonical, file_hash, strict_loads, verify as verify_execution,
)
...
def _load_manifest(path: str | Path) -> dict:
    ...
    if (manifest.get("schema_version") != "rq1-workflow/4"
...
def _verified_unauthorized_effect(report: dict):
    ...
    if value is None or value is False:
        # v4 observes a registered positive lower bound, not all possible
...
def _reported_episode(folder: Path, manifest: dict, cfg: dict, allocation: dict,
    ...
    report_path, anchor_path = folder / "report.json", folder / "execution-anchor.json"
    ...
    report, anchor = _object(report_path), _object(anchor_path)
    if file_hash(report_path) != status["report_sha256"]:
    ...
    if (report.get("schema_version") != "rq1-episode-report/4"
...
def _episode(root: Path, manifest: dict, cfg: dict, expected_world: str,
    ...
    if allocation is None:
        if any((folder / name).exists() for name in ("report.json", "status.json", "execution-anchor.json")):
    ...
    if run_status != "completed" and not (folder / "report.json").exists():
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Read-only honest/malicious paired pilot analyzer")
