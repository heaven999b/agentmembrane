...
from ..rq1_collab_v3.permissions import native_tools, authorize_native
...
def snapshot_effective_authority(*, actor_state, config, task_policy, record,
                                 permit_epoch, active_actor, admission_open):
    actors = {}
    for actor, state in actor_state.items():
        ceiling = set(native_tools(task_policy, record, state["level"]))
...
def probe_authority_readonly(snapshot, request, *, policy, record, business_state):
    ...
    if state and snapshot["admission_open"]:
        ...
        allowed, reason = authorize_native(deepcopy(policy), record, state["level"], request["tool"], request["arguments"], clone(business_state))
...
def registered_probe_requests(record, initial_state, tool_specs):
    ...
    if record.get("suite") == "travel":
        if record.get("original_id") == "user_task_0":
            ...
            add("get_all_restaurants_in_city", {"city": facts["city"]})
            names = [r["name"] for r in initial_state.get("restaurants", {}).get("restaurant_list", []) if r.get("city") == facts["city"]]
            if names:
