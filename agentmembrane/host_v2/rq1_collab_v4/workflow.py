...
from .contract import (PROTOCOL, REGIMES, TOPOLOGIES, LEVELS, clone, digest,
...
from .contract import system_spec, MEMORY_PROFILE, PROFILES
...
def code_fingerprint():
    files = []
    for package in ("rq1_collab_v1", "rq1_collab_v3", "rq1_collab_v4", "rq1_scorecard_v5", "rq1_measurement_v1"):
...
def prepare(output, *, tasks, repeats=1, mode="engineering", qualified=QUALIFIED,
            goals=GOALS, inventory=INVENTORY, system_profile=MEMORY_PROFILE,
            models_file=None, budget_file=None, topologies=TOPOLOGIES,
    ...
    manifest = {"schema_version": "rq1-workflow/4", "protocol_version": PROTOCOL,
...
def load_manifest(path):
    ...
    if manifest.get("schema_version") != "rq1-workflow/4" or digest(unsigned) != manifest.get("manifest_sha256"):
    ...
    if "episode_budget" in manifest or "budget_source" in manifest:
        ...
        if (type(origin) is not dict
                ...
                or origin["selection"] != "explicit_bounded_diagnostic_budget"
...
def run_cell(manifest_path, episode_id, output, *, execute_live=False, inventory=INVENTORY):
    ...
    if remote:
        ...
        try:
            ...
            return status
        if inv.get("base_url") != proxy_base() or not ready["listed"] or not fresh:
    ...
    try:
        ...
        if remote:
            ...
            driver = RoleModelDriver(cfg, role_prompts(
                cfg, rec["public"]["user_request"], rec["public"]["goal"]["goal"],
        ...
        capture = run_episode(cfg, bundle, native, driver, collector, system_profile=manifest["system_spec"]["system_profile"])
...
def main():
    ...
    prep.add_argument("--system-profile", choices=PROFILES, default=MEMORY_PROFILE)
    ...
    if args.command == "prepare":
        result = prepare(args.output, tasks=args.tasks, repeats=args.repeats, mode=args.mode,
                         ...
                         budget_file=args.budget_file, topologies=args.topologies,
