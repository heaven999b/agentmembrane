"""Zero-token authority and tool-knowledge baseline gate for RQ1 v5.
...
GATE_ID = "rq1-public-agentdojo-tool-knowledge-integration-v5"
GATE_STATUS = "tool_knowledge_native_parity_established_7of7"
ARTIFACT_TYPE = "agentmembrane_rq1_agentdojo_tool_knowledge_gate_v5"
...
_IMPLEMENTATION_PATHS = (
    ...
    "agentmembrane/host_v2/rq1_public_agentdojo_multi_v5/selector.py",
...
def _admission_purity() -> dict[str, Any]:
    ...
    value = {
        ...
        "protected_monkeypatch_probe_count": 7,
...
def _selector_audit() -> dict[str, Any]:
    ...
    if len(cells) != 28 or [row.execution_ordinal for row in cells] != list(range(1, 29)):
    ...
    if any(
        set(counter) != coordinate_names
        or sum(counter.values()) != 7
        or min(counter.values()) != 1
    ...
    value = {
        "workflow_count": 7, "cell_count": 28,
        "execution_ordinals": list(range(1, 29)),
...
def run_zero_token_integration_gate() -> dict[str, Any]:
    """Run all 28 real resets and return one strict canonical receipt."""
    ...
    with _deny_network() as network:
        for cell in cells:
            ...
            try:
                ...
                session.cleanup()
    workflows = [
        _finalize_workflow(states[row.source_task_id])
        for row in EXPERIMENT_WORKFLOWS
    ]
    ...
    counts = {
        ...
        "real_native_session_resets": 28, "authority_workflows": 7,
        "protected_monkeypatch_denial_probes": 7,
...
def validate_integration_gate(record: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if (value.get("schema_version") != 1 or value.get("artifact_type") != ARTIFACT_TYPE
            ...
            or value.get("workflow_count") != 7 or value.get("cell_count") != 28):
    ...
    expected_counts = {
        **{key: 0 for key in _ZERO_KEYS}, "real_native_session_resets": 28,
        "authority_workflows": 7, "protected_monkeypatch_denial_probes": 7,
        "native_final_checker_calls_for_admission": 0,
    }
    ...
    if not isinstance(source_workflows, list) or len(source_workflows) != 7:
    ...
    if (purity.get("static_forbidden_call_sites") != []
            ...
            or purity.get("protected_monkeypatch_probe_count") != 7
    ...
    if (selector.get("workflow_count") != 7 or selector.get("cell_count") != 28
            or selector.get("execution_ordinals") != list(range(1, 29))
            or selector.get("latin_rotation_counts") != {
                "0": 2, "1": 2, "2": 2, "3": 1
            }
    ...
    for workflow, row in zip(EXPERIMENT_WORKFLOWS, evidence):
