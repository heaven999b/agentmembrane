...
class RuntimeMonitor:
    ...
    def state(self, point, actor, internal_used, delegation_count, **extra):
        ...
        authority = snapshot_effective_authority(actor_state=self.actors, config=self.cfg,
            ...
            profile=self.spec["system_profile"])
        ...
        if self.trace["authority_registrations"] and probe_key != self.last_probe_state:
            ...
            probe = self.emit("authority_probe_batch", {"authority_event_id": ar["event_id"],
                ...
                "business_state_sha256": world_hash,
    ...
    def native_transition(self, call, *, deadline_monotonic=None):
        ...
        if call.get("after") is not None:
            ...
            if call.get("before") != call["after"]:
                # A native mutation can change object-level authorization with
                # no change to actor/tool grants. Re-probe the registered partial
                # request partition against the trusted post-call world.
                actor = call["actor"]
                authority = snapshot_effective_authority(
                    actor_state=self.actors, config=self.cfg,
                    task_policy=self.policy, record=self.record,
                    permit_epoch=self.epoch, active_actor=actor,
                    admission_open=self.open, profile=self.spec["system_profile"])
                ar = self.emit("authority_snapshot", {
                    "point": "after_native", "native_call_id": call["call_id"],
                    "state": authority,
                    "delegation_event_ids": list(self.grant_event_ids)}, actor)
                self.trace["authority_snapshots"].append(ar)
                receipts = []
                for recipient in sorted(authority["actors"]):
                    for request in self.probe_requests:
                        receipt = probe_authority_readonly(
                            authority, {"actor": recipient,
                                "tool": request["tool"],
                                "arguments": request["arguments"]},
                            policy=self.policy, record=self.record,
                            business_state=call["after"],
                            profile=self.spec["system_profile"])
                        receipts.append({**receipt, "probe_id": request["probe_id"]})
                probe = self.emit("authority_probe_batch", {
                    "authority_event_id": ar["event_id"],
                    "native_call_id": call["call_id"],
                    "snapshot_sha256": authority["state_sha256"],
                    "business_state_sha256": digest(call["after"]),
                    "registration_event_id": self.trace["authority_registrations"][0]["event_id"],
                    "receipts": receipts, "error_type": None,
                    "complete_parameter_partition": False}, actor,
                    [ar["event_id"]])
                self.trace["authority_probes"].append(probe)
