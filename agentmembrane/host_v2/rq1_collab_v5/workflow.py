...
def code_fingerprint():
    files = []
    for package in ("rq1_collab_v1", "rq1_collab_v3", "rq1_collab_v4", "rq1_collab_v5", "rq1_scorecard_v5", "rq1_measurement_v1"):
...
def prepare(output, *, tasks, repeats=1, mode="engineering", qualified=QUALIFIED,
    ...
    if budget_file is not None:
        ...
        budget_source = {"path": str(budget_path), "sha256": file_hash(budget_path),
                         ...
                         "selection": "explicit_v5_bounded_phase_budget"}
        if budget != FIXED_BUDGET:
            raise ValueError("exact_v5_bounded_budget_file_required")
    root = Path(output)
    ...
    manifest = {"schema_version": "rq1-workflow/5", "protocol_version": PROTOCOL,
...
def load_manifest(path):
    ...
    if manifest.get("schema_version") != "rq1-workflow/5" or digest(unsigned) != manifest.get("manifest_sha256"):
    ...
    if (manifest.get("phase_schedule") != PHASE_SCHEDULE
        ...
        raise ValueError("v5_phase_or_regime_binding_mismatch")
    ...
    if "episode_budget" in manifest or "budget_source" in manifest:
        ...
        if (type(origin) is not dict
                ...
                or origin["selection"] != "explicit_v5_bounded_phase_budget"
...
def run_cell(manifest_path, episode_id, output, *, execute_live=False, inventory=INVENTORY):
    ...
    try:
        ...
        if remote:
            transport = dedicated_proxy_transport(
