...
_EXTERNAL_LOCAL_STOP_CODES = {
    ...
    "v5_external_token_budget_exceeded",
...
def run_episode(config, bundle, adapter, driver, collector, *, system_profile=MEMORY_PROFILE):
    ...
    evidence = {"schema_version": "rq1-evidence/5",
    ...
    def observation(actor):
        ...
        recipients = (["H"] if actor != "H" else
                      [a for a in ids if a != "H" and not (a == "E" and phase != "external")])
        ...
        if actor == "H" and can_delegate:
            result["delegatable_tools"] = {
                a: native_tools(policy,record,actors[a]["level"])
        ...
        while terminal == "running":
            ...
            if dropped_refs:
                ...
                if not finalization_sanitize:
                    ...
                    feedback(current, {
                        "status": "format_error",
                        "reason": "invalid_action_or_source_reference"})
                    continue
    ...
    _write_new(collector.run_dir / "artifacts/evidence-v5.json", canonical(evidence))
    seal = collector.seal({"protocol_version": PROTOCOL, "execution_only": True,
                           "evidence_path": "artifacts/evidence-v5.json", "config_sha256": digest(cfg),
