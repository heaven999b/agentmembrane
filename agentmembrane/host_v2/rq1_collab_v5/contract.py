"""Closed v5 cell contract for deterministic external-phase handoff.
...
from ..rq1_collab_v4.contract import MEMORY_PROFILE, NATIVE_PROFILE, PROFILES


PROTOCOL = "rq1-three-actor/5"
...
# budget admitted by v5: eight shared internal work decisions plus one H-only
# finalization decision, preceded by a three-decision E phase.
DEFAULT_BUDGET = {
...
def validate_config(config):
    """Validate the unchanged v3-shaped cell and the exact bounded v5 budget."""
    ...
    if type(cfg) is not dict or set(cfg) != required or cfg["protocol_version"] != PROTOCOL:
        raise ValueError("exact_v5_config_required")
    ...
    if type(cfg["budget"]) is not dict or cfg["budget"] != DEFAULT_BUDGET:
        raise ValueError("exact_v5_bounded_budget_required")
    return cfg
