"""Durable 48-cell executor for the canonical RQ1 AgentDojo multi-source bank.
...
from ..rq1_smoke import ExactRQ1SolMaxClient
...
SCHEMA_VERSION = 1
PROFILE_ARTIFACT_TYPE = "agentmembrane_rq1_multi_source_real_profile_v3"
AUTHORIZATION_ARTIFACT_TYPE = "agentmembrane_rq1_multi_source_real_authorization_v3"
CONSUMPTION_ARTIFACT_TYPE = (
    "agentmembrane_rq1_multi_source_authorization_consumption_v3"
)
CELL_ARTIFACT_TYPE = "agentmembrane_rq1_multi_source_cell_v3"
REPORT_ARTIFACT_TYPE = "agentmembrane_rq1_multi_source_report_v3"
CALL_RESERVATION_ARTIFACT_TYPE = "agentmembrane_rq1_multi_source_call_reservation_v3"
CALL_TERMINAL_ARTIFACT_TYPE = "agentmembrane_rq1_multi_source_call_terminal_v3"
CELL_COUNT = 48
MAX_TURNS_PER_CELL = 6
GLOBAL_CALL_CAP = 288
CANARY_CELL_COUNT = 4
CANARY_CALL_CAP = 24
CONTINUATION_CELL_COUNT = 44
CONTINUATION_CALL_CAP = 264
CANARY_ORDINALS = (1, 13, 25, 37)
...
_CODE_RELATIVE_PATHS = {
    ...
    "analysis": Path("agentmembrane/host_v2/rq1_public_agentdojo_multi_v3/analysis.py"),
    "contracts": Path(
        "agentmembrane/host_v2/rq1_public_agentdojo_multi_v3/contracts.py"
    ),
    ...
    "executor": Path("agentmembrane/host_v2/rq1_public_agentdojo_multi_v3/executor.py"),
    "gate": Path("agentmembrane/host_v2/rq1_public_agentdojo_multi_v3/gate.py"),
    "provider_agent": Path(
        "agentmembrane/host_v2/rq1_public_agentdojo_multi_v3/provider_agent.py"
    ),
    ...
    "runtime": Path("agentmembrane/host_v2/rq1_public_agentdojo_multi_v3/runtime.py"),
    "selector": Path("agentmembrane/host_v2/rq1_public_agentdojo_multi_v3/selector.py"),
}
...
def _adapter_preflight_under_pinned_python(root: Path) -> dict[str, Any]:
    ...
    script = (
        ...
        "print(json.dumps(AgentDojoAdapter().preflight(),sort_keys=True,"
        "separators=(',',':')))\n"
    )
    try:
...
def _runtime_binding(
    ...
    try:
        validated_receipt = load_provisioning_receipt(
            receipt_path,
            expectation=agentdojo_runtime_expectation(repo_root=root),
            expected_receipt_sha256=_sha(receipt_path),
            verify_live_files=True,
        )
        import_preflight = validate_agentdojo_runtime_receipt(
    ...
    adapter_preflight = (
        dict(AgentDojoAdapter().preflight())
    ...
    if (
        ...
        or adapter_preflight.get("adapter_id") != AgentDojoAdapter.adapter_id
...
def _schedule() -> tuple[EngineeringCell, ...]:
    ...
    if (
        ...
        raise MultiSourceExecutorError(
            "selector did not return the frozen 48-cell schedule"
        )
    return cells
...
def _canary_cells() -> tuple[EngineeringCell, ...]:
    ...
    try:
        ...
        raise MultiSourceExecutorError(
            "cross-domain canary ordinal is unavailable"
        ) from exc
    if (
        len(cells) != CANARY_CELL_COUNT
        or len({cell.domain for cell in cells}) != CANARY_CELL_COUNT
    ):
        raise MultiSourceExecutorError("canary must contain one cell from every domain")
    return cells
...
def _continuation_cells() -> tuple[EngineeringCell, ...]:
    ...
    if len(cells) != CONTINUATION_CELL_COUNT:
        raise MultiSourceExecutorError("continuation schedule must contain 44 cells")
    return cells
...
def build_profile_document(
    ...
    *,
    ...
    profile = {
        ...
        "profile_id": "canonical-rq1-agentdojo-multi-source-v3",
...
def build_authorization_document(
    *, profile: Mapping[str, Any], namespace: str
    ...
    return {
        ...
        "authorization_id": f"{namespace}-authorization-v3",
        "decision": "AUTHORIZE_EXACT_48_CELL_ENGINEERING_RUN",
...
def _run_cell(
    *,
    ...
    try:
        ...
        if production and type(client) is not ExactRQ1SolMaxClient:
            raise MultiSourceGlobalFatal(
...
def _canary_report(
    ...
    return {
        ...
        "operational_gate_contract": {
            "requires_four_immutable_cell_records": True,
...
def _final_report(root: Path) -> dict[str, Any]:
    ...
    report = {
        ...
        "evidence_eligibility": {
            ...
            "requires_48_immutable_unique_cell_records": True,
...
def _production_client_factory(_: EngineeringCell) -> CompletionClient:
    client = ExactRQ1SolMaxClient.from_local_config(timeout_seconds=120.0)
    if type(client) is not ExactRQ1SolMaxClient or client.calls_made != 0:
...
def _execute_continuation(
    *,
    ...
    if (
        ...
        raise MultiSourceGlobalFatal("canary report differs from its four cell records")
...
def execute_canary_v3(
    *,
    ...
    """Execute the cross-domain production canary with exact real bindings."""
...
def execute_canary_test_only_v3(
    *,
    repo_root: Path,
    output_root: Path,
    namespace: str,
...
def resume_canary_test_only_v3(
    *,
    repo_root: Path,
    namespace_path: Path,
    gate_receipt: Mapping[str, Any] | Path | str,
...
def execute_continuation_test_only_v3(
    *,
    repo_root: Path,
    namespace_path: Path,
    gate_receipt: Mapping[str, Any] | Path | str,
...
__all__ = [
    ...
    "execute_canary_v3",
    "execute_canary_test_only_v3",
    "execute_continuation_v3",
    "execute_continuation_test_only_v3",
    "resume_canary_v3",
    "resume_canary_test_only_v3",
]
